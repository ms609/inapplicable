...
```{r}
better.tree <- BasicSearch(tree=nj.tree, dataset=my.phyDat, Rearrange=TreeSearch::RootedNNI)
...
```{r}
better.tree <- BasicSearch(rooted.tree, my.phyDat)
```

Using NNI might help to explore the region of treespace close to the local optimum:
```{r}
better.tree <- BasicSearch(better.tree, my.phyDat, Rearrange=TreeSearch::RootedNNI)
...
```{r}
best.tree <- RatchetSearch(better.tree, my.phyDat, verbosity=3)
