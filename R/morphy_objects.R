...
#'
#' @parem weight A vector listing the new weights to be applied to each character
#' @param morphyObj A morphy object created with \code{\link{LoadMorphy}}
#' @return Morphy error codes generated by each character. (Can be translated
#'         using \code{\link{mpl_translate_error}})
#' 
#' @author Martin R. Smith
#' @export
SetMorphyWeights <- function (weight, morphyObj) {
  if (length(weight) != mpl_get_num_charac(morphyObj)) stop("Number of weights not equal to number of character entries")
