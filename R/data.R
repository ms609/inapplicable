#' Raw data from Zhang et al 2016
#'
#' @template LoboMods
#'
#' @keywords datasets
#' @source Zhang X-G, Smith MR, Yang J, Hou J-B. 2016 Onychophoran-like musculature in a phosphatized Cambrian lobopodian. Biol. Lett. 12: 20160492. \url{http://dx.doi.org/10.1098/rsbl.2016.0492}
"Lobo.data"

#' Data from Zhang et al 2016, in phyDat format
#'
#' @template LoboMods
#'
#' @format \describe{
#'    \item{Lobo.data}{a list, each entry reporting the textual representation of the tokens for a named taxon}
#'    \item{Lobo.phy}{The same data, in \code{phyDat} format.}
#' }
#' 
#' @keywords datasets
#' @source Zhang X-G, Smith MR, Yang J, Hou J-B. 2016 Onychophoran-like musculature in a phosphatized Cambrian lobopodian. Biol. Lett. 12: 20160492. \url{http://dx.doi.org/10.1098/rsbl.2016.0492}
"Lobo.phy"
