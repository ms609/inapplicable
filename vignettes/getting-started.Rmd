...
```{r}
set.seed(1) # Set random seed so that random functions will generate consistent output in this document
...
par(mar=rep(0.25, 4), cex=0.75) # set plot parameters so trees are almost legible
plot(rooted.tree)
```
...
```{r}
best.tree <- Ratchet(better.tree, my.phyDat, verbosity=0)
